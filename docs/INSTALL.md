...
このプロジェクトでは `PPCustomZoom_TemporaryKey.pfx` を参照しています。

手元に証明書がない場合は、次のいずれかを行ってください。

- ローカル開発用のテスト証明書を作成して設定する
- Visual Studio 側で署名設定を見直す
...
- Excel の Ribbon に `VTab` 関連 UI が表示される
- 左側に縦型タブ pane が表示される
...
ClickOnce / VSTO の署名設定や証明書が原因のことが多いです。まずは証明書設定を見直してください。
