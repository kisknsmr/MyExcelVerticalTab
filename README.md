...
現在の構成では、`PPCustomZoom_TemporaryKey.pfx` のようなリポジトリ内の証明書ファイルには依存していません。
開発用のビルドと発行は、Windows の現在のユーザー証明書ストア `CurrentUser\My` に登録されたテスト用コード署名証明書を使う前提です。

そのため、少なくともこの開発 PC では、証明書問題は解消した状態で `Debug` / `Release` のビルドと `Publish` が通ります。
ただし、別の PC や別ユーザー環境で同じことを行う場合は、その環境にも証明書を用意するか、正式なコード署名証明書へ置き換える必要があります。
